...
void ARRDE::initialize() {
    ...
    double multiplier; 
    if (logeta>2.) {
        multiplier = 2.0+  5.756* std::pow(logeta-2.0, 1.609);
    } else {
        multiplier = 2.0;
    };
    const double defaultPopulation    = std::clamp(dimension*multiplier, 4.0, 3000.0);
...
void ARRDE::updateParameterMemory() {
    ...
    if (memoryIndex == (memorySize-1)) {
        ...
        M_F[memoryIndex] = 0.9;
        //if (!first_run) M_F[memoryIndex] = 0.5;
        memoryIndex = 0;
    } else memoryIndex++;
...
void ARRDE::resampleControlParameters() {
    ...
    for (size_t i = 0; i < popSize; ++i) {
        ...
        if (first_run){
            if (Nevals < 0.25*0.9*maxevals && newCR[i] < 0.7) newCR[i] = 0.7;
            if (Nevals < 0.5*0.9*maxevals && newCR[i] < 0.6) newCR[i] = 0.6;
            if (Nevals < 0.6*0.9*maxevals && newF[i] > 0.7) newF[i] = 0.7;
        };
    }
