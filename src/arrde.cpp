...
void ARRDE::adaptParameters() {
    ...
    if ( calcStdDev(fitness)/calcMean(fitness)<=reltol || Nevals>=strartRefine*maxevals){ 
        ...
        if ((first_run && Nevals<strartRefine*maxevals)  || (bestOverall<=best_fitness  && Nevals<strartRefine*maxevals && Nrestart<maxRestart)) {
           // std::cout << "Restarted after " << Nevals << " " << bestOverall << " "<< best_fitness << " " << population.size() << " " << reltol<< "\n";
            ...
            memoryIndex=0;
            Fw= 0.6+0.7*Nevals/(strartRefine*maxevals);
    ...
    if (!S_CR.empty()) {
        ...
        if ( M_CR[memoryIndex] <0.5){
            M_CR[memoryIndex] = std::min(M_CR[memoryIndex]*2.0, 1.0);
            hasUpdate=true;
        };
        if (M_F[memoryIndex]>0.1) {
            M_F[memoryIndex] = std::max(0.5*M_F[memoryIndex], 0.1);
            hasUpdate=true;
        };
    }
    sr=0.0;
    ...
    for (int i = 0; i < population.size(); ++i) {
        double fraction = 0.2;
        if (refine) fraction = 0.2+0.3*Nevals/(strartRefine*maxevals);
        if (restart) fraction = 0.2+0.3*Nevals/(strartRefine*maxevals);
