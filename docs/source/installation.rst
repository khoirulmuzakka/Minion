...
- Apple Silicon: ``minion-arm64.tar.gz``
- Intel: ``minion-x64.tar.gz``
...
For a user-local installation, first extract the archive into a temporary
directory:

.. code-block:: shell

   mkdir -p /tmp/minion_pkg "$HOME/.local/minion"
   tar -xzf minion-<arch>.tar.gz -C /tmp/minion_pkg
...
If you install into a non-default prefix such as ``$HOME/.local/minion``, add
that prefix to ``CMAKE_PREFIX_PATH`` when configuring your project.
